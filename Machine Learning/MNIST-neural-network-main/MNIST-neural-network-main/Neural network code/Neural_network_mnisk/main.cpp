...
void runMnist_NN()
{
	...
	Network test_NN(784, 60, 10);// network slayer sizes 
	...
	for (int i = 40001; i != 41001; i++) // 1000 validation of dataset
	{
		std::cout << "\nEpoch:" << i << ": label:" << trainSet.getDataSet()[i].getLabel() << '\n';
...
/* ============================================================= //
*this is a test set on Mnist mini that only has upto 9 inputs 
* can be tested using just one number
* ===============================================================*/
void runTestSet()
{
	...
	Network test_NN(784, 15, 10);// network layer sizes 
	...
	while (!end)
	{
		for (int i = 0; i != 100; i++)
		{
			std::cout << "\ndEpoch:" << i << ": label:" << trainSet.getDataSet()[2].getLabel() << '\n';
			test_NN.trainNetwork(trainSet.getDataSet()[2].getPixels(), trainSet.getDataSet()[2].getLabel());
